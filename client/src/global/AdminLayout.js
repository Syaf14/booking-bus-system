...
import axios from 'axios'; // Import axios for logout call
    ...
    <div className="container-fluid p-0">
      <div className="row g-0">
        ...
        <div className="col-md-9 col-lg-10 bg-light">
          {/* TOP NAV BAR - LOGOUT BUTTON ADDED HERE */}
          <nav className="navbar navbar-expand navbar-light bg-white border-bottom px-4 shadow-sm d-flex justify-content-between" style={{ height: "60px" }}>
            <span className="navbar-brand mb-0 h6 text-muted">
                {location.pathname.replace('/', '').replace('admin-', '').replace(/-/g, ' ').toUpperCase() || "DASHBOARD"}
            </span>
            
            <button className="btn btn-sm btn-outline-danger px-3 fw-bold rounded-pill" onClick={handleLogout}>
